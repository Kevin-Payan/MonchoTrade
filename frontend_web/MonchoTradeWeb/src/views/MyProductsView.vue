...
const fetchProducts = async () => {
  try {
    ...
    const response = await axios.get(`${appsettings.apiUrl}/user/products/${userId}`, appsettings.axiosConfig)
    products.value = response.data.map(product => ({
     ...product,
     imageUrl: product.imageUrl ? `${appsettings.apiUrl}${product.imageUrl}` : null
   }))
...
const updateProductImage = async () => {
  ...
  try {
    ...
    await axios.put(
      ...
      formData, appsettings.axiosMultiFormDataConfig);
