            ...
            <form @submit.prevent="submitProposal" class="space-y-6">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Selecciona el artículo que deseas intercambiar
                </label>
                <div class="space-y-4">
...
const fetchMyItems = async () => {
  try {
    const response = await axios.get(
      `${appsettings.apiUrl}${appsettings.userRoute}/products/${YourId}`,
      {
        headers: {
          "Content-Type": "application/json",
        }
...
const submitProposal = async () => {
  ...
  try {
    const proposal = {
      ...
      initiatorProductId: selectedItemId.value,
      receiverProductId: ItemYouWant,
      notes: message.value
    }
